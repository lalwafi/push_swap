...
class RectDisplayWidget(Widget):
    ...
    def draw_rectangles(self, *largs):
        ...
        offset = 0
        """
        if len(self.stack_orig) > 0:
            #b_val = [(num - mn) / ((mx - mn) + 1) for num in self.stack_b]
            with self.canvas:
                for num in self.stack_orig:
                    rank = self.get_rank(num)
                    self.set_color(rank)
                    rect = StackRectangle(num, rank,
                                          self.get_rect_pos(self.center_x, offset),
                                          self.get_rect_size(rank))
                    self.stack_b.append(rect)
                    offset += offset_add
                for valb in b_val:
                    # rectb = [hw, vi, hw + 10 + valb * (hw - (hw/3)), vi + wh / hm]
                    self.set_color(valb)
                    self.rects.append(
                        Rectangle(pos=(hw, (wh - vi - (wh / hm)) + self.y),
                                  size=(hw + 10 + valb * (hw - (hw/3)),
                                        wh / hm)))
                    vi += wh / hm
        """
